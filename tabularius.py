...
@myself.event
async def custom_send_message(channel, message):
    while len(message) > 2000:
        await myself.send_message(channel, message[:2000])
        message = message[2000:]
    await myself.send_message(channel, message)
    ...
    if myself.user.mention in message.content and not message.author.bot and not message.channel.is_private:
        ...
        if "start" in message.content.lower():
            if not offeringreads in message.author.roles:
                ...
                await custom_send_message(message.channel, message.author.mention+" is now offering readings.")
            ...
            if offeringreads in message.author.roles:
                ...
                await custom_send_message(message.channel, message.author.mention+" is no longer offering readings.")

@myself.event
async def on_member_join(member):
    channels = member.server.channels
    botlog = [channel for channel in channels if channel.id == "311888667947827200"][0]
    await custom_send_message(botlog, member.mention+" has joined the server.")

@myself.event
async def on_member_remove(member):
    channels = member.server.channels
    botlog = [channel for channel in channels if channel.id == "311888667947827200"][0]
    await custom_send_message(botlog, member.mention + " has left the server.")

@myself.event
async def on_message_delete(message):
    channels = message.server.channels
    botlog = [channel for channel in channels if channel.id == "311888667947827200"][0]
    await custom_send_message(botlog, message.author.mention + " has deleted a message from "+message.channel.name+": " + message.content)
